...
esp32_ip = "http://192.168.4.1"  # Change to your ESP32's IP
distance_threshold = 30
angle_threshold = 10
target_id = 1
moving = False

# Function to send HTTP request to ESP32 without parameters
...
def fetch_ir_remote():
    global target_id
    try:
        response = requests.get(f"{esp32_ip}/get_ir_code", timeout=0.5)
        if response.status_code == 200:
            ir_code = response.text.strip()
            print(f"Received IR Code: {ir_code}")
            try:
                new_id = int(ir_code, 16)  # Convert hex to int
                if 1 <= new_id <= 5:
                    set_target_id(new_id)
            except ValueError:
                pass
    except requests.RequestException:
        print("Failed to fetch IR signal.")
...
while True:
    ...
    if ids is not None:
        ...
        for i in range(len(ids)):
            ...
            cv2.putText(frame, f"ID: {ids[i][0]}, Angle: {int(angle)}°", (x + 10, y - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
    ...
    # Fetch IR remote input once per frame
    fetch_ir_remote()
